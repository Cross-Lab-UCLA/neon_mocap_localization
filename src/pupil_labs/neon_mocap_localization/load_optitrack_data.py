...
# Obtained from:
# https://github.com/cmuphyscomp/hmv-s16/blob/master/python/optitrack/csv_reader.py
# Modified by RJE (01.2026)

"""\

optitrack.csv_reader : a plain-Python parser for reading Optitrack CSV files in
version 1.2 or 1.21 format.
...
class Marker:
    def __init__(self, label, ID):
        self.label = label
        self.ID = ID
        self.positions = []  # list with one element per frame, either None or
        # [x,y,z] float lists
...
class RigidBody:
    ...
    def __init__(self, label, ID):
        self.label = label
        self.ID = ID
        # list with one element per frame, either None
        # or [x,y,z] float lists
        self.positions = []
        # list with one element per frame, either None
        # or [x,y,z,w] float lists
        self.rotations = []
        self.times = []  # list with one element per frame with the capture time
        return
...
class Take:
    ...
    def __init__(self):
        ...
        # dict of RigidBody objects, indexed by asset
        # name string
        self.rigid_bodies = {}
        self.markers = {}
        ...
        self._raw_info = {}
        # line 3: raw column types for all data columns (not including frame and
        # time column)
        self._raw_types = []
        # line 4: raw asset names for all data columns (not including frame and
        # time column)
        self._raw_labels = []
        # line 6: raw field types for all data columns (not including frame and
        # time column)
        self._raw_fields = []
        # line 7: raw axis designators for all data columns (not including frame
        # and time column)
        self._raw_axes = []
        # names of all ignored objects
    ...
    # ================================================================
    def _read_header(self, stream, verbose=False):  # noqa: C901
        # Line 1 consists of a series of token, value pairs.
        line1 = next(stream)
        assert line1[0] == "Format Version", f"Unrecognized header cell: {line1[0]}"
        format_ = line1[1]

        assert format_ == "1.21" or format_ == "1.2" or format_ == "1.24", (
            f"Unsupported format version: {line1[1]}"
        )
        ...
        # Line 2 is blank.
        line2 = next(stream)
        assert len(line2) == 0, f"Expected blank second header line, found {line2}."
        ...
        # Line 4 designates the asset labels for each column (e.g. 'Rigid Body 1',
        # or whatever name was assigned)
        line4 = next(stream)
        ...
        # Line 6 designates the data type for each column: Rotation, Position,
        # Error Per Marker, Marker Quality
        line6 = next(stream)
        ...
        # Process lines 3-7 at once, creating named objects to receive each frame
        # of data for supported asset types.
        for col, asset_type, label, ID, field, axis in zip(
            ...
            if asset_type == "Rigid Body" or asset_type == "Rigid Body Marker":
                ...
                # create a column map entry for each marker axis
                if field == "Position":
                    axis_index = {"X": 0, "Y": 1, "Z": 2}[axis]
        ...
        # the actual frame data begins with line 8, one frame per line,
    ...
    def _read_data(self, stream, verbose=False):
        ...
        for row_num, row in enumerate(stream):
            ...
            # if verbose: print "Processing row_num %d, frame_num %d, time %f."
            # % (row_num, frame_num, frame_t)
            ...
            for mapping in self._column_map:
                # each mapping is a namedtuple with a setter method, column index,
                # and axis name
